...
class AlterWarehouseAddColumns extends Migration
{
    ...
    public function up()
    {
        Schema::table('warehouse', function (Blueprint $table) {
            $table->tinyInteger('type')->default(null)->nullable()->after('name')->comment('类型1-加盟2-自营');
            $table->tinyInteger('is_center')->default(2)->nullable()->after('type')->comment('是否为分拨中心1-是2-否');
            $table->string('acceptance_type')->default('')->nullable()->after('is_center')->comment('承接类型列表1-取件2-派件3-直送');
