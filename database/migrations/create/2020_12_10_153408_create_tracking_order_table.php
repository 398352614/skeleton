...
class CreateTrackingOrderTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('tracking_order', function (Blueprint $table) {
            ...
            $table->string('warehouse_fullname', 50)->default('')->nullable()->comment('仓库-姓名');
            $table->string('warehouse_phone', 50)->default('')->nullable()->comment('仓库-手机号码');
            $table->string('warehouse_country', 50)->default('')->nullable()->comment('仓库-国家');
            $table->string('warehouse_post_code', 50)->default('')->nullable()->comment('仓库-邮编');
            $table->string('warehouse_house_number', 50)->default('')->nullable()->comment('仓库-门牌号');
            $table->string('warehouse_city', 50)->default('')->nullable()->comment('仓库-城市');
            $table->string('warehouse_street', 50)->default('')->nullable()->comment('仓库-街道');
            $table->string('warehouse_address', 250)->default('')->nullable()->comment('仓库-地址');
            $table->string('warehouse_lon', 50)->default('')->nullable()->comment('仓库-经度');
            $table->string('warehouse_lat', 50)->default('')->nullable()->comment('仓库-纬度');
