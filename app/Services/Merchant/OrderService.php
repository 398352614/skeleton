...
class OrderService extends BaseService
{
    ...
    public function store($params, $orderSource = BaseConstService::ORDER_SOURCE_1)
    {
        ...
        //订单第三方日志记录
        ($orderSource == BaseConstService::ORDER_SOURCE_3) && ThirdPartyLogService::storeAll(auth()->user()->merchant_id, ['order_no' => $params['order_no']], BaseConstService::NOTIFY_STORE_ORDER, '订单预约成功，外部订单号:' . $params['out_order_no'] ?? '' . '，预约线路:' . $tour['line_name'] . '，预约日期:' . $params['execution_date']);
        return [
    ...
    {
        $info = $this->getInfoByIdOfStatus($id, true, [BaseConstService::ORDER_STATUS_1, BaseConstService::ORDER_STATUS_2, BaseConstService::ORDER_STATUS_3, BaseConstService::ORDER_STATUS_6, BaseConstService::ORDER_STATUS_7]);
