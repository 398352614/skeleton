...
class RouteTrackingService extends BaseService
{
    ...
    {
        if ($this->formData['is_online'] == BaseConstService::YES) {
            $where = ['status' => BaseConstService::TOUR_STATUS_4];
        } else {
            $where = ['status' => ['<>', BaseConstService::TOUR_STATUS_4]];
        }
        if (!empty($this->formData['driver_name'])) {
            $where[] = ['driver_name' => $this->formData['driver_name']];
        }
...
//            }
        $info = $this->getDriverService()->getList($where, ['*'], false);
        if (empty($info)) {
            throw new BusinessLogicException('暂无车辆信息');
        }
        for ($i = 0, $j = count($info); $i < $j; $i++) {
            $info[$i]['driver_id'] = $info[$i]['id'];
            $info[$i]['driver_name'] = $info[$i]['fullname'];
            $info[$i]['driver_phone'] = $info[$i]['phone'];
            $info[$i]['car_no'] = $info[$k]['id'] = $info[$k]['lat'] = $info[$k]['lon'] = $info[$k]['line_name'] = $info[$k]['time'] = $info[$k]['tour_no'] = '';
