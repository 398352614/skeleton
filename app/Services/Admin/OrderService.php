    ...
    function addAllItemList($params, $status = BaseConstService::ORDER_STATUS_1)
    {
        ...
        if (!empty($params['package_list'])) {
            ...
            $packageList = collect($params['package_list'])->map(function ($item, $key) use ($params, $array, $status) {
                $collectItem = collect($item)->only(['name', 'express_first_no', 'express_second_no', 'out_order_no', 'feature_logo', 'weight', 'actual_weight', 'settlement_amount', 'count_settlement_amount', 'expect_quantity', 'remark', 'is_auth', 'expiration_date','stage']);
                return $collectItem
