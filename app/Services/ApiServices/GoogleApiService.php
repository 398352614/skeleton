...
class GoogleApiService
{
    ...
    public function InitTour(Tour $tour)
    {
        ...
        $res = $this->client->postJson($this->url . $api . $this->makeSign(now()->timestamp - 5.9 * 3600), $params);
    ...
    {
        $api = '/api/line-info';
        $path = $this->url . $api . $this->makeSign(now()->timestamp - 5.9 * 3600) . "&line_code=$lineCode";
    ...
    public function PushDriverLocation($data)
    {
        ...
        $res = $this->client->postJson($this->url . $api . $this->makeSign(now()->timestamp - 7 * 3600), $data);
    ...
    public function updateTour(Tour $tour, $nextCode, $driverLocation = [])
    {
        ...
        $this->client->postJson($this->url . $api . $this->makeSign(now()->timestamp - 5.9 * 3600), $params);
        FactoryInstanceTrait::getInstance(ApiTimesService::class)->timesCount('api_distance_times', $tour->company_id);
        //更新距离和时间
