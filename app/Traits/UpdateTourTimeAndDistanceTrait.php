    ...
    public function updateTourTimeAndDistance($tour): bool
    {
        ...
        try {
            ...
            foreach ($data['loc_res'] as $key => $res) {
                ...
                //若站点未签收,则智能调度
                if (!empty($tourBatch->toArray()) && in_array(intval($tourBatch->status), [BaseConstService::BATCH_WAIT_ASSIGN, BaseConstService::BATCH_ASSIGNED, BaseConstService::BATCH_WAIT_OUT, BaseConstService::BATCH_DELIVERING])) {
