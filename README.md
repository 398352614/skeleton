...
### 2. 技术介绍
该项目于2019年年末进行筹备，2020年1月进行正式开发，首要需求为满足荷兰方的运输管理需求，分担ERP项目的部分功能，其次计划将该产品推向国内，服务国内小型运输公司。本项目采取前后端分离模式，后端基于php+mysql+redis利用Laravel框架开发，前端基于HTML+css+javascript利用vue框架开发。  
php版本：7.4.6  
mysql版本：5.7.11  
redis版本：  1
laravel版本：6.18.27
...
正式服H5端地址：[https://tms-h5.eutechne.com](https://tms-h5.eutechne.com)  
正式服客户端地址：[https://tms-consumer.eutechne.com](https://tms-consumer.eutechne.com)
...
开发服货主端地址：[https://dev-tms-h5.nle-tech.com](https://dev-tms-h5.nle-tech.com)  
开发服H5端地址：[https://dev-tms-consumerconsumer.nle-tech.com](https://dev-tms-consumer.nle-tech.com)


港服管理员端地址：[https://admin.rrtms.com](https://admin.rrtms.com)
港服管理员端地址：[https://order.rrtms.com](https://admin.rrtms.com)
港服H5端地址：[https://h5.rrtms.com](https://h5.rrtms.com)
港服客户端地址：[https://consumer.rrtms.com](https://consumer.rrtms.com)
...
按顺序分别为定时任务，数据库，服务器，php命令行，php扩展，数据库网页版，缓存。其中还有supervisor及wkhtmltopdf安装在php-cli的虚拟容器中。
...
通过克隆，下载到服务器上。
注意：应在docker环境文件夹同层新建www文件夹，再在www文件夹内git clone克隆项目代码。  代码分支形式为deploy01，deploy02...以数字最大的为主分支。
```gitexclude
...
#### 1.3 功能外部连接
TMS系统目前利用到的第三方接口有谷歌地图API，腾讯地图API，postcode.nl，routeXL接口，paypal支付接口。智能优化，距离计算等功能的正常运作需要以上第三方接口生效，因此请在项目根目录下的.env文件中对各个接口进行正确配置。  
除此之外，本项目有自动翻译的功能，如果需要用到该功能，还需调用百度翻译API。
谷歌地图接口：[https://maps.googleapis.com/maps/api](https://maps.googleapis.com/maps/api)  
谷歌地图文档：[https://developers.google.com/maps/documentation](https://developers.google.com/maps/documentation)  
腾讯地图接口：[https://apis.map.qq.com/ws/distance/v1/optimal_order](https://apis.map.qq.com/ws/distance/v1/optimal_order)  
腾讯地图文档：[https://lbs.qq.com](https://lbs.qq.com)  
postcode.nl接口：[https://api.postcode.nl/rest](https://api.postcode.nl/rest)  
百度翻译接口：[http://api.fanyi.baidu.com/api/trans/vip/translate](http://api.fanyi.baidu.com/api/trans/vip/translate)
百度翻译文档：[https://api.fanyi.baidu.com/doc/21](https://api.fanyi.baidu.com/doc/21)
routeXL接口：[https://api.routexl.com](https://api.routexl.com)
paypal文档[https://paypal.github.io/PayPal-PHP-SDK/sample](https://paypal.github.io/PayPal-PHP-SDK/sample)
paypal接口[https://paypal.github.io/PayPal-PHP-SDK/sample](https://paypal.github.io/PayPal-PHP-SDK/sample)
### 2. 业务流程
...
### 3. 自定义命令
在Laravel框架下，拥有便利的自定义artisan命令。在项目目录下的app/Console/Commands文目录内，存放有所有的自定义artisan命令。按类型分为不同文件夹。命令命名规则为动词+名词

#### 3.1 Admin后台
#### 3.1.1 socket通讯推送
```
php artisan push:admin {id : the admin u_id} {type : the push type}
```
#### 3.1.2 订单签收重推
```
php artisan retry:order {--order_no= : order_no} {--tour_no= : tour_no}
```
#### 3.1.3 智能优化解锁任务
```
...
##### 3.3.1 删除所有缓存
```
php artisan cache:destroy
```
所有缓存项都有重新写入缓存的机制，因此无须担心删除缓存会造成影响，唯一的影响就是下次读取缓存的时候会重建缓存导致请求变慢。
#### 3.4 Cron定时任务
##### 3.4.1分钟
```
php artisan cron:minute
```
每分钟执行的定时任务。
#### 3.5 Data数据
##### 3.5.1 fix型命令
修复型命令，主要是版本之间的新老数据的修复，一般可重复操作。
##### 3.5.2 init型命令
初始化型命令，主要为初始化某些数据，一般不可重复操作，初始化操作会导致旧数据丢失。
##### 3.5.3 fill型命令（未来计划删除）
##### 3.5.4 reset型命令
重置型命令，主要为重置某些静态数据，执行前请确认是否重置，重置操作会导致旧数据丢失。
#### 3.5.1 抛错自动翻译
```
...
##### 4.2.3 新增app/Exports目录（未来将移入manager）
该目录下存放所有的导出模板。
##### 4.2.4 新增app/Imports目录（未来将移入manager）
该目录下存放所有的导入模板。
##### 4.2.5 新增app/Mail目录（未来将移入manager）
该目录下存放邮件发送相关代码。
##### 4.2.6 新增app/Worker目录（未来将移入manager）
该目录下存放即时通讯相关代码。
##### 4.2.7 新增config/tms.php文件
该文件储存环境配置路径，所有业务代码只能通过该文件访问.env里的环境配置。这样做是为了安全性与统一管理。
该目录下存放即时通讯相关代码。
##### 4.3.2 新增app/Manager目录
参照大厂经验，将通用的功能性类，集中放置在该文件夹下，例如curl包，excel的导入导出，第三方支付等。使用manager时，需要new这个类，而不是像service一样使用单例。
#### 4.3 设计通例
##### 4.3.1 Service相互之间的调用
在一个service中，通过getInstance方法构造单例，从而调用其他service。controller等其他类大多数无法进行同级调用。
##### 4.3.2 基类继承
在service目录下，有一个BaseService作为基类，该类中存有较为通用的方法。其他所有service类都继承这个基类，又根据业务需求分别拥有特殊方法。并且，可以通过复写基类中的方法对其通用方法进行定制。除了service，其他所有的类都可以以此方法构建通用基类，提高代码复用率。如果某一类service除了BaseService外，仍然有较多相同代码，可以在BaseService与Serice之间再加一层通用类，例如由于按区域分配订单与按邮编分配订单这两个服务，拥有部分共同的处理步骤，所以在AreaService与BaseService之间，新建BaseLineService。
##### 4.3.3 上传文件目录结构
...
### 1 composer包说明
#### 1.1 fideloper/proxy，代理,
#### 1.2 laravel/framework，laravel框架核心
#### 1.3 laravel/tinker，命令行调试工具
#### 1.4 barryvdh/laravel-ide-helper。开发助手，提供门面跳转等功能
#### 1.5 facade/ignition。提供错误页面视图
#### 1.6 fzaninotto/faker。造假数据工具
#### 1.7 mockery/mockery。基于phpunit的单元测试工具
#### 1.8 nunomaduro/collision。静态代码审查工具
#### 1.9 phpunit/phpunit。单元测试工具
#### 1.10 ext-curl。curl扩展。命令行HTTP请求工具。
#### 1.11 guzzlehttp/guzzle。微框架HTTP请求工具。
#### 1.12 ext-json。json扩展。
#### 1.13 aws/aws-sdk-php。亚马逊扩展。
#### 1.14 barryvdh/laravel-snappy。pdf扩展。
#### 1.15 doctrine/dbal。查询构造器ORM扩展。
#### 1.16 firebase/php-jwt。JSON Web Token验证扩展。
#### 1.17 intervention/image。图片处理扩展。
#### 1.18 jiaxincui/closure-table。树形结构扩展。
#### 1.19 maatwebsite/excel。excel扩展。
#### 1.20 milon/barcode。条形码扩展。
#### 1.21 mjaschen/phpgeo。经纬度计算扩展。
#### 1.22 overtrue/laravel-wechat。微信扩展。
#### 1.23 pda/pheanstalk。消息队列扩展。
#### 1.24 php-http/guzzle6-adapter。Guzzle 6 适配器。
#### 1.25 predis/predis。redis扩展。
#### 1.26 spatie/laravel-permission。权限扩展。
#### 1.27 firebase/php-jwt。JSON Web Token验证扩展。
#### 1.28 vinkla/hashids。哈希ID扩展。
#### 1.29 workerman/gateway-worker。TCP长连接即时通讯工具。
#### 1.30 textalk/websocket。TCP长连接即时通讯工具。
#### 1.31 ext-mysql_xdevapi。数据库连接工具。
#### 1.32 laravel/telescope。提供请求，日志，抛错等信息展示页面的调试工具。
#### 1.33 paypal/rest-api-sdk-php。paypalSDK。
### 2 新增权限
执行集成命令
```
php artisan init:base-permission
```
如果用户较多，该过程较长，请耐心等待。集成命令分为3步：写入基础权限表-》补齐角色-》补齐权限-》进行缓存
